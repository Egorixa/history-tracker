...
export async function getConfig(windowId) {
  ...
  const keys = [SHARED_KEYS.apiBase, SHARED_KEYS.blacklist, pk.apiToken, pk.userId, pk.username, pk.autopostChannels];
...
export async function saveAuth(windowId, { apiBase, apiToken, userId, username }) {
  const pk = perWindowKeys(windowId);
  const set = {
...
export async function saveSettings(windowId, { autopostChannels, blacklist }) {
  const pk = perWindowKeys(windowId);
  const set = {};
  if (autopostChannels !== undefined) set[pk.autopostChannels] = autopostChannels;
